...
//      reaches MIN_SENTENCE_LEN characters.
//    * A `.` additionally has to survive `isSentenceBoundary` — abbreviations
//      and initials are not sentence ends.
//
//  The MIN_SENTENCE_LEN guard is about position only; it was never abbreviation
//  handling despite what this comment used to claim. "Mr. Smith" past character
//  20 split happily, so "an order from Lt. Commander …" was spoken as
//  "an order from Lt." followed by a multi-second stall while the rest of the
//  line synthesized.
...
nonisolated final class SentenceDetector {
    ...
    /// Titles that are essentially always followed by a name, so their `.` is
    /// never a sentence end ("Lt. Commander", "Dr. Crusher").
    ///
    /// Deliberately NOT derived from `TextNormalizer.abbreviations`. That map
    /// exists to expand abbreviations for pronunciation, which is a different
    /// question from "does this period end a sentence", and most of it answers
    /// the second one wrong: `Inc.` `Corp.` `Ltd.` `Co.` `St.` `min.` `max.`
    /// `avg.` `est.` `approx.` `Jr.` and all twelve months routinely DO close a
    /// sentence ("…filed with Acme Corp. Then he left."). Suppressing those
    /// merges two sentences into one chunk and delays the first spoken audio —
    /// the same symptom this whole fix exists to remove, just in the other
    /// direction. Titles are the only entries where the name-continuation is
    /// reliable enough to be worth the trade.
    ...
    /// Whether the terminator at `index` — already known to be followed by
    /// whitespace — actually ends a sentence.
    ///
    /// One rule: a known abbreviation or a lone initial before it means it
    /// didn't ("Lt. Commander", "J. Smith"). `!` and `?` always split.
    //
    // Land mine: do NOT add "and the next word is lowercase means it didn't".
    // That looks like a free win and is not. It suppresses EVERY period before a
    // lowercase word with no cap, so a turn written in lowercase — a normal
    // register for the Relaxed / Butterfly Chaser presets — yields zero
    // sentences and speaks nothing until `flush()` at end of stream. It also
    // makes streaming and batch disagree: mid-stream the following word is not
    // buffered yet, so the check is skipped and the split happens, while the
    // same text re-fed whole does not split. `truncatedSpokenText` re-segments
    // in batch and takes `prefix(spokenSentences)` from the streaming count, so
    // the divergence makes barge-in keep lines the user never heard.
    private static func isSentenceBoundary(_ chars: [Character], at index: Int) -> Bool {
        guard chars[index] == "." else { return true }

        // Bounded — no abbreviation in the table exceeds "LCpl". Unbounded, this
        // walked the whole preceding run (base64 blobs, CJK with no spaces).
        var start = index
