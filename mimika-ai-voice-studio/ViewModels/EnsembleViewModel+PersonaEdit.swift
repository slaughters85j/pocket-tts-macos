...
//
//  Post-acceptance persona editing (Cast & Settings' pencil): the persona editor works on local copies and lands them here on close — name and script mutate the LIVE cast, then one commit persists to the saved cast. Sibling file to EnsembleViewModel.swift per the file-size guideline, matching the existing +Context/+Director/+Export pattern.
//
//  WP-CAST-1 also lives here: manual add/remove of cast members after the persona writer finishes (roster was previously fixed at accept time).
//
...
extension EnsembleViewModel {
    ...
    /// Set a persona's display name on the LIVE cast without persisting — the save is deferred to `commitPersonaEdit` on editor close. Only sensible before the conversation starts (the UI gates it): past turns keep whatever name they were spoken under.
    func setPersonaName(at index: Int, name: String) {
        guard cast.indices.contains(index) else { return }
        cast[index].name = name
    }

    /// Set a persona's system-prompt script on the LIVE cast without persisting — same deferred-save contract as `setPersonaName`. (Runtime `Persona` names the field `systemPrompt`; the persisted `EnsemblePersona` calls it `personaPrompt`.)
    func setPersonaPrompt(at index: Int, prompt: String) {
        guard cast.indices.contains(index) else { return }
        cast[index].systemPrompt = prompt
    }

    /// Persist any pending `setPersonaName` / `setPersonaPrompt` edits to the saved cast. Called once, when the persona editor closes.
    ...
    /// Update the live scene string and persist to the saved cast (export uses the same fields; next turns pick them up via `framedSystemPrompt`).
    ...
    private func persistSceneAndMood() {
        ...
        saved.scene = scene
        saved.mood = mood
        // Cast display name tracks scene when we have one (same convention as `persistCast` / import).
    ...
    /// Set how the cast addresses the human. Empty → display "You" / model "Guest" (same convention as the New Cast wizard). Mirrors display into `modelName` when the user picks a real proper noun so the LLM never sees the pronoun "You" as a speaker label. Also keeps the composer character picker in sync.
    ...
    /// Switch the active human identity to a name already on the roster (or any string). Overrides Cast & Settings and persists to the cast.
    func selectUserCharacter(_ name: String) {
        updateUserPeerName(name)
    }

    /// Add a new character name, make it active, and remember it on the picker. Empty / whitespace-only fails. Case-insensitive duplicates just select the existing entry.
    @discardableResult
    ...
    /// Seed the picker from the active peer (cast load / new cast). Real proper nouns only — skips the default "You" pronoun.
    ...
    /// Insert `name` at the front of the roster if missing; drop oldest when over the cap. Does not change the active peer.
    ...
    /// Remove a speaker by index from the live cast + saved cast. Refuses when it would leave fewer than `minCastSize` members.
    @discardableResult
    func removeCastMember(at index: Int) -> Bool {
        ...
        // Prefer identity-ish match (name + voice) so a reordered store row can't delete the wrong persona; fall back to full resync.
    ...
    /// Arm a one-shot exit for `speakerID`: they speak next with the reason, then leave the cast. Remaining speakers get a public departure note. Cuts any in-flight line immediately so the exit does not wait.
    @discardableResult
    ...
    /// Arm a one-shot cast-specific direction: that speaker is forced next with the instruction injected and Strict sampling so compliance is likelier. Unlike Boot they stay in the cast. Instruction must be non-empty. Cuts any in-flight line immediately so the note lands on the next pick.
    @discardableResult
